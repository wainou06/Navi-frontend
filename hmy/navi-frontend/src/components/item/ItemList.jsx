...
const ItemsList = () => {
   ...
   const loadItems = (newFilters = filters) => {
      dispatch(fetchItemsThunk(newFilters))
   }
   ...
   const handleDelete = async () => {
      if (deleteDialog.item) {
         try {
            await dispatch(deleteItemThunk(deleteDialog.item.id)).unwrap()
                  ...
                  <div className="products-grid">
                     {items.map((item) => (
                        <div key={item.id} className="product-card">
                           ...
                           <div className="product-image">{item.images && item.images.length > 0 ? <img src={`${import.meta.env.VITE_APP_API_URL || 'http://localhost:8000'}/uploads${item.images[0].url}`} alt={item.name} /> : <div className="placeholder-image">이미지</div>}</div>
